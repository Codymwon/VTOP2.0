...
### Color Palette (Official Live VTOP Brand Palette)

| Role | Hex | Source / CSS Variable |
|------|-----|----------------------|
| Primary Header Blue | `#2455A3` | `rgb(36, 85, 163)` (`.headerBackgroundColor` / `.footercolor`) |
| Primary Interactive Blue | `#176CB8` | `rgb(23, 108, 184)` (`.primaryTextColor1`) |
| Secondary Sky Blue | `#3497DB` | `rgba(52, 151, 219, 1)` (Header gradient mid) |
| Deep Header Accent | `#2874A6` | `rgba(40, 116, 166, 1)` (Header gradient deep) |
| Student Card Border | `#3C8DBC` | `rgb(60, 141, 188)` (`.cardStudent` / `.navyBorder`) |
| Employee Accent Gold | `#D4AC0D` | `rgb(212, 172, 13)` (`.employeeTextColor` / `.employeeBtnColor`) |
| Parent Portal Green | `#18AD4C` | `#18ad4c` (`.greenText` / `.cardParent`) |
| Alumni Portal Cyan | `#5DADE2` | `rgb(93, 173, 226)` (`.cardAlumni`) |
| Spotlight Maroon | `#900C3F` | `rgb(144, 12, 63)` (`.hightlight1`) |
| University Crest Maroon | `#7D0E14` | Official VIT-AP crest emblem on light background |
| Background Canvas | `#F8FAFC` | `--color-background` |
| Card Background | `#FFFFFF` | `--color-card` |
| Text Primary | `#0F172A` | High contrast body text |
| Border | `#CBD5E1` | Standard card / control border |

**Color Notes:** Official VIT-AP VTOP Live Palette extracted from `https://vtop.vitap.ac.in/vtop/open/page` and `PreLogin.css`.
